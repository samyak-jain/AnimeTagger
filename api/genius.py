...
class GENIUS(API):
    def __init__(self):
        ...
        self.SEARCH_URL = f"{self.BASE_URL}/search?access_token={os.getenv('GENIUS_TOKEN')}"

    def query(self, user_query: str, session: ClientSession) -> Task:
        return asyncio.create_task(self.fetch(f"{self.SEARCH_URL}?={user_query}", session))
    ...
    def album(self, response_list: List[Optional[Dict[str, Any]]]) -> \
        ...
        for response in response_list:
            ...
            for song in response["response"]["hits"]:
                ...
                if song_name is not None and artists is not None and album_art is not None:
